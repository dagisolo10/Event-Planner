...
function Sidebar({
    ...
    if (collapsible === "none") {
        return (
            <div data-slot="sidebar" className={cn("text-sidebar-foreground bg-background flex h-full w-(--sidebar-width) flex-col", className)} {...props}>
                {children}
            </div>
        );
    }
...
function SidebarSeparator({ className, ...props }: React.ComponentProps<typeof Separator>) {
    return <Separator data-slot="sidebar-separator" data-sidebar="separator" className={cn("mx-2 w-auto bg-zinc-300 dark:bg-zinc-700", className)} {...props} />;
}
...
const sidebarMenuButtonVariants = cva(
    "peer/menu-button data-[active=true]:dark:bg-sidebar-accent data-[active=true]:bg-primary data-[active=true]:text-zinc-100! data-[active=true]:dark:bg-accent dark:text-zinc-400 dark:hover:text-zinc-100 text-zinc-600 data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:font-medium group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
    {
        variants: {
            variant: {
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
    return (
        <ul
            ...
            className={cn("border-sidebar-border mx-3.5 flex min-w-0 translate-x-px flex-col gap-2 border-l px-3 py-px pt-2", "group-data-[collapsible=icon]:hidden", className)}
            {...props}
        />
    );
}
...
function SidebarMenuSubButton({ asChild = false, size = "md", isActive = false, className, ...props }: React.ComponentProps<"a"> & { asChild?: boolean; size?: "sm" | "md"; isActive?: boolean }) {
    ...
    return (
        <Comp
            ...
            className={cn(
                "ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 text-zinc-600 outline-hidden focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 dark:text-zinc-400 dark:hover:text-zinc-100 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
                "data-[active=true]:[&>svg]:text-background data-[active=true]:bg-primary data-[active=true]:text-zinc-100!",
                "data-[active=true]:dark:bg-sidebar-accent data-[active=true]:[&>svg]:dark:text-zinc-100",
                size === "sm" && "text-xs",
                size === "md" && "text-sm",
                "group-data-[collapsible=icon]:hidden",
                className,
