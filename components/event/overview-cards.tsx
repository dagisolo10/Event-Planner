...
export default function OverviewCard({ dark = false, label, value, icon: Icon, color, paymentCompletion, darkLabel }: CardProp) {
    ...
    if (dark)
        return (
            <div className="group relative md:col-span-2 overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-950 p-6 text-zinc-100 shadow-2xl transition-all duration-500 hover:border-zinc-700">
                    ...
                    <div className="mb-6 flex items-center justify-between">
                        <div className="space-y-1">
                            <div className="flex items-center gap-2">
                                ...
                                <p className="text-ss font-black tracking-[0.2em] text-zinc-500 uppercase">{darkLabel}</p>
                            </div>
                        ...
                        <div className="text-right">
                            ...
                            <p className="text-ss font-bold tracking-widest text-zinc-500 uppercase">{isOverpaid ? "Over-Limit Threshold" : "Reconciliation"}</p>
                        </div>
                    </div>
                        ...
                        <div className="flex justify-between">
                            {isOverpaid ? (
                                <div className="flex animate-pulse items-center gap-1 text-rose-500">
                                    ...
                                    <span className="text-ss font-black tracking-tighter uppercase">System Alert: Excess Payment</span>
                                </div>
                            ) : (
                                <span className="text-ss text-muted-foreground font-bold tracking-tight uppercase">Verified Ledger Status</span>
                            )}
                        </div>
                    </div>
                </div>
    ...
    return (
        <div className="group text-foreground relative overflow-hidden rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md dark:border-zinc-800 dark:bg-zinc-950">
            <div className="relative z-10">
                ...
                <p className="text-ss text-muted-foreground font-black tracking-wider uppercase">{label}</p>
            ...
            <div className={cn("absolute bottom-0 left-0 h-0.5 w-full opacity-0 transition-opacity duration-500 group-hover:opacity-100", color === "text-muted-foreground" ? "bg-black dark:bg-white" : color.replace("text-", "bg-"))} />
        </div>
    );
}
